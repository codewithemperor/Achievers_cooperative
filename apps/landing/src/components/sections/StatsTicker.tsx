...
export default function StatsTicker() {
  ...
  return (
    <section className="overflow-hidden bg-primary-700 py-5 text-white dark:bg-primary-900">
      <div className="marquee-track">
        {items.map((stat, i) => (
          <div
            key={i}
            className="flex shrink-0 items-center gap-8 px-10 text-white"
          >
            <span className="font-display text-2xl font-semibold">{stat.value}</span>
            <span className="text-sm text-white/75">{stat.label}</span>
            <span className="text-lg text-white/35" aria-hidden="true">
              ✦
            </span>
          </div>
        ))}
