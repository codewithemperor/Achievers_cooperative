      ...
      <section className="rounded-[28px] bg-white/92 p-4 shadow-[0_22px_48px_rgba(15,23,42,0.08)] dark:bg-[color:rgba(15,23,42,0.72)]">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-text-400">
              Your key member identity details are stored locally after the
              first fetch for quicker revisits.
            </p>
          </div>
          <button
        ...
        <div className="mt-4 grid gap-3">
          {[
            ...
            <div
              key={item.label}
              className="rounded-[24px] border border-[var(--background-200)] px-4 py-4 dark:border-white/10"
            >
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-text-400">
                {item.label}
              </p>
              <p className="mt-2 text-sm font-medium text-text-800 dark:text-text-100">
                {item.value}
              </p>
            </div>
          ))}
        ...
        <form className="grid gap-4" onSubmit={handleSubmit(onSubmitProfile)}>
          ...
          <div className="grid gap-2">
            <label
              ...
              htmlFor="member-profile-picture"
            >
              Profile picture
            </label>
            <input
